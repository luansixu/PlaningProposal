...
const DebugOverlay = (function () {
  ...
  function _ensure() {
    ...
    desc.textContent = '请复制诊断信息发给开发/自己定位；或刷新重试。';
    ...
    const btnCopy = mkBtn('复制诊断信息', async function () {
      ...
      try {
        ...
        // clipboard 可能在 file:// 下不可用，降级为选中文本
        Logger && Logger.warn && Logger.warn('剪贴板写入失败，已在页面中选中文本，请手动复制。', e);
  ...
  self.buildDiagnosticsText = function () {
    ...
    parts.push(Logger && Logger.formatRecent ? Logger.formatRecent(60) : '(Logger unavailable)');
    parts.push('');
    parts.push('== Overlay ==');
  ...
  self.installGlobalHandlers = function () {
    window.addEventListener('error', function (e) {
      // e.error 可能是 undefined（跨域/某些浏览器），尽量保留信息
